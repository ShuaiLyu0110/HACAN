...
from .objectives import compute_irtr_recall, compute_irtr_val, compute_irtr_test_gl, compute_irtr_testv2
...
def epoch_eval_irtr(pl_module, is_test=False):
    phase = "train" if pl_module.training else "val"

    if (not pl_module.training) and (not is_test):
        (ir_r1, ir_r5, ir_r10, ir_r20, ir_r50, ir_r70, ir_r100, tr_r1, tr_r5, tr_r10, tr_r20, tr_r50, tr_r70, tr_r100) = compute_irtr_val(pl_module)
        print("Image to text: %.1f, %.1f, %.1f, %.1f, %.1f, %.1f, %.1f" % (ir_r1, ir_r5, ir_r10, ir_r20, ir_r50, ir_r70, ir_r100))
        print("Text to image: %.1f, %.1f, %.1f, %.1f, %.1f, %.1f, %.1f" % (tr_r1, tr_r5, tr_r10, tr_r20, tr_r50, tr_r70, tr_r100))
        pl_module.logger.experiment.add_scalar(
            "recalls/ir_r1", ir_r1, pl_module.global_step
        )
        pl_module.logger.experiment.add_scalar(
